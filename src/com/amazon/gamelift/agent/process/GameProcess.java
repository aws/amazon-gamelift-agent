...
/**
 * GameProcess is a wrapper around the Java 9 Process that a game server build
...
public class GameProcess {
    ...
    // the build and with Amazon GameLift services. This is not the PID from the Compute System -
    ...
    // Status field indicating the current state of where the process is at in its lifecycle
     ...
     * using the GameProcessConfiguration provided in the constructor in order to start up
     * a game server process. The internalProcess field will be set to the Process
    ...
    /**
     * Attaches a method to the internal process' onExit() future so that the cleanup/reporting tasks can be performed
     * when the process exits.
     */
    public void handleProcessExit(final BiConsumer<Process, GameProcess> onExitMethod) {
        if (internalProcess != null) {
